...
<link href="/resources/user/css/bookPurchase.css" rel="stylesheet"
	type="text/css">
<link rel="shortcut icon" href="https://ifh.cc/g/74QNWX.png"
	type="image/x-icon" />
...
<body>
	...
	<!-- 본문 s-->
	<div class="container">
		<form action="/member/subookEnd" id="bookPurchase" name="bookPurchase"
			method="post">
			<input type="hidden" id="tditSeq" name="tditSeq"
				value="<c:out value="${item.tditSeq}"/>"> <input
				type="hidden" id="rtCount" name="rtCount"> <input
				type="hidden" id="rtFinalPrice" name="rtFinalPrice"> <input
				type="hidden" id="rtPoint" name="rtPoint">
			<div class="row">
				<div class="col-md-9">
					<div class="px-0 mt-4">
						<h3 style="font-weight: bold; color: darkblue;" class="mb-4">주문/결제</h3>
						<div style="width: 90%;" class="mx-auto">
							<p class="mb-2" style="font-weight: bold; font-size: 23px;">
								주문자</p>
							<table class="table mb-4">
								<tr>
									<td><c:out value="${itemMember.kbmmName}" /></td>
									<td><c:set var="numberPhone"
											value="${itemMember.kbmpNumberFull}" /> <c:choose>
						...
						<div style="width: 90%;" class="mx-auto mt-5">
							<p class="mb-4" style="font-weight: bold; font-size: 23px;">
								배송정보</p>
							<ul class="nav nav-tabs" id="myTab" role="tablist">
								<li class="nav-item" role="presentation">
									<button class="nav-link active" id="home-tab"
										data-bs-toggle="tab" data-bs-target="#home" type="button"
										role="tab" aria-controls="home" aria-selected="true">국내배송</button>
								</li>
								<li class="nav-item" role="presentation">
									<button class="nav-link" id="profile-tab" data-bs-toggle="tab"
										data-bs-target="#profile" type="button" role="tab"
										aria-controls="profile" aria-selected="false">해외배송</button>
								</li>
							</ul>
								...
								<div class="tab-pane fade show active" id="home" role="tabpanel"
									...
									<table class="table mb-4 mx-auto">
										<tr>
											<th class="w-25">이름</th>
											<td><input type="text"
												class="form-control form-control-sm" id="kbmmName"
												name="kbmmName" value="${itemMember.kbmmName}"></td>
										</tr>
										<tr>
											...
											<td><c:choose>
													<c:when test="${fn:length(numberPhone) eq 10 }">
														<input type="text" class="form-control form-control-sm"
															id="kbmpNumberFull" name="kbmpNumberFull"
															value="<c:out value="${fn:substring(numberPhone,0,3)}"/> - <c:out value="${fn:substring(numberPhone,3,6)}"/> - <c:out value="${fn:substring(numberPhone,6,10)}"/>">
													</c:when>
													<c:otherwise>
														<input type="text" class="form-control form-control-sm"
															id="kbmpNumberFull" name="kbmpNumberFull"
															value="<c:out value="${fn:substring(numberPhone,0,3)}"/> - <c:out value="${fn:substring(numberPhone,3,7)}"/> - <c:out value="${fn:substring(numberPhone,7,11)}"/>">
													</c:otherwise>
												</c:choose></td>
										</tr>
										<tr>
											...
											<td><div class="input-group">
													<input class="form-control form-control-sm mb-1"
														type="text" id="kbmaZipcode" name="kbmaZipcode"
														placeholder="우편번호" value="${itemMember.kbmaZipcode}"
														readonly>
													<button class="btn btn-outline-secondary btn-sm mb-1"
														type="button" id="btnAddress">
														<i class="fas fa-search"></i>
													</button>
													<button class="btn btn-outline-secondary btn-sm mb-1"
														type="button" id="btnClear">
														<i class="fas fa-solid fa-x"></i>
													</button>
												</div> <input type="text"
												class="form-control form-control-sm mb-1" id="kbmaAddress1"
												name="kbmaAddress1" value="${itemMember.kbmaAddress1}"
												placeholder="기본주소"> <input type="text"
												class="form-control form-control-sm mb-1" id="kbmaAddress2"
												name="kbmaAddress2" value="${itemMember.kbmaAddress2}"
												placeholder="상세주소"></td>
										</tr>
									</table>
								</div>
						...
						<div style="width: 90%;" class="mx-auto mt-5">
							...
							<table class="table mb-4">
								...
								<tr>
									<td><c:forEach items="${listUploaded}" var="itemUploaded"
											...
											<c:if test="${itemUploaded.defaultNy eq 1}">
												<img class="mx-auto " alt=""
													...
													style="width: 125px; height: 180px; float: start; margin: 6px;">
											</c:if>
										</c:forEach></td>
									<td style="vertical-align: middle;"><c:set
											var="listCateDepth1"
											value="${CateServiceImpl.selelctListCachedCate('1')}" /> <c:forEach
											items="${listCateDepth1}" var="itemCate"
											varStatus="statusCate">
											<c:if test="${item.tditItemCate eq itemCate.ifctSeq}">
												<p>[${itemCate.ifctName}]</p>
											</c:if>
										</c:forEach>
										<p style="font-weight: bold;">
											<c:out value="${item.tditTitle}" />
										</p></td>
									<td style="vertical-align: middle;"><c:set
											var="listCodeDiscount"
											value="${CodeServiceImpl.selectListCachedCode('13')}" /> 정가 :
										<fmt:formatNumber value="${item.tditPrice}" />원<br> <c:forEach
											items="${listCodeDiscount}" var="itemDiscount"
											varStatus="statusDiscount">
											<c:if test="${item.tditDiscountCd eq itemDiscount.ifcdOrder}">
												<span style="font-weight: bold;">[<c:out
														value="${itemDiscount.ifcdName}" /> <fmt:formatNumber
														value="${itemDiscount.ifcdReferenceI2*item.tditPrice}" />원
													인하]
												</span>
												<br>
												<br>
										판매가 : <span
													style="color: #F84450; font-size: 20px; font-weight: bold;">
													<fmt:formatNumber
														value="${item.tditPrice-(itemDiscount.ifcdReferenceI2*item.tditPrice)}" />원
												</span>
											</c:if>
										</c:forEach></td>
									<td style="vertical-align: middle;"><%=request.getParameter("result")%>
										개</td>
									<td style="vertical-align: middle;"><p class="my-auto">수북문고
											배송</p></td>
								</tr>
							</table>
						...
						<div style="width: 90%" class="mx-auto mt-5">
							...
							<table class="table mb-4">
								...
								<tr>
									...
									<td style=""><button type="button" id="coupon1"
											value="6000">사용하기</button></td>
								</tr>
								<tr>
									...
									<td style=""><button type="button" id="coupon2"
											value="5000">사용하기</button></td>
								</tr>
								<tr>
									...
									<td style=""><button type="button" id="coupon3"
											value="2000">사용하기</button></td>
								</tr>
							</table>
						</div>
						...
						<div style="width: 90%" class="mx-auto mt-5">
							...
							<table class="table mb-4">
								...
								<tr>
									...
									<td style=""><fmt:formatDate value="${ymd}"
											pattern="yyyy-MM-dd" /></td>
								</tr>
								<caption style="font-size: 13px;">※ 적립금은 적립 후 1년까지
									사용가능 합니다</caption>
							</table>
						</div>
						<div style="width: 90%" class="mx-auto mt-5">
							<%-- <c:set var="CodePay" value="${CodeServiceImpl.selectListCachedCode('18')}"/> --%>
							<p class="mb-3" style="font-weight: bold; font-size: 23px;">
								결제</p>
							<table class="table mb-4">
								<tr>
								...
								<tr>
									<td><input class="form-check-input" type="radio"
										name="rtPayment" id="rtPayment1" value="1" disabled> <label
										class="form-check-label" for="rtPayment1">신용카드</label></td>
									<td><input class="form-check-input" type="radio"
										name="rtPayment" id="rtPayment2" value="2" disabled> <label
										class="form-check-label" for="rtPayment2">계좌이체</label></td>
									<td><input class="form-check-input" type="radio"
										name="rtPayment" id="rtPayment3" value="3" disabled> <label
										class="form-check-label" for="rtPayment3">핸드폰결제</label></td>
									<td><input class="form-check-input" type="radio"
										name="rtPayment" id="rtPayment4" value="4" disabled> <label
										class="form-check-label" for="rtPayment4">네이버페이</label></td>
									<td><input class="form-check-input" type="radio"
										name="rtPayment" id="rtPayment5" value="5"> <label
										class="form-check-label" for="rtPayment5">카카오페이</label></td>
								</tr>
							</table>
						</div>
						<div style="width: 90%" class="mx-auto mt-5">
							<p class="mb-4" style="font-weight: bold; font-size: 23px;">
								도서 소득공제</p>
							<ul>
								<li style="font-size: 13px;">카드결제는 카드 명의자 기준으로, 현금결제는
									개인공제용으로 현금영수증 신청한 기준으로 국세청에 자동 반영됩니다.</li>
								<li style="font-size: 13px;">도서 소득공제 가능 결제수단 : 신용카드(개인카드에
									한함), 카카오페이, 네이버페이, 삼성페이, PAYCO, 토스, CHAI, 실시간계좌이체, 온라인입금, 예치금,
									교보e캐시(충전e캐시에 한함), 해피머니상품권, 컬쳐캐쉬, 기프트카드</li>
							</ul>
						</div>
					</div>
				</div>
				...
				<div class="col-md-3 d-none d-md-block">
					<div class="px-0 mt-4" id="floatMenu">
						<div class="header">
							<p
								style="font-size: 26px; color: darkblue; font-weight: bold; text-align: center;">총
								결제 정보</p>
						</div>
						<hr>
						<div class="content">
							...
							<p id="pInfo">
								배송비 : <span id="fee"></span>원
							</p>
							<p id="pInfo">
								상품금액 : <span id="totalPrice"></span>원
							</p>
						</div>
						<hr>
						<div class="discount">
							<p id="pDis">
								할인금액 : <span style="font-size: 25px; color: green;"><b
									id="couponPrice"></b></span> 원
							</p>
						</div>
						<hr>
						<div class="finalSum">
							<c:forEach items="${listCodeDiscount}" var="itemDiscount"
								varStatus="statusDiscount">
								<c:if test="${item.tditDiscountCd eq itemDiscount.ifcdOrder}">
									<p id="pFin">
										최종 결제금액 : <span style="font-size: 24px; color: red;"><b
											id="totalPrice2"></b>원</span>
									</p>
								</c:if>
							</c:forEach>
						</div>
						...
						<div class="savingPoint">
							<p>
								※ 적립예정 포인트 : <span id="point1"></span> p
							</p>
						</div>
						<div>
							<button type="button" class="btn w-100" name="btn-purchase"
								id="btn-purchase">바로구매</button>
	...
	<script type="text/javascript"
		src="//dapi.kakao.com/v2/maps/sdk.js?appkey=6ec915718ae8d23e16c65e0f6d22a62e&libraries=services"></script>
	<!-- iamport.payment.js -->
	<script type="text/javascript"
